...
parameters {
  ...
  // ---- Bandit stickiness (subject-level; bonus for repeating last chosen bandit) ----
...
transformed parameters {
  ...
  vector[nSub] kappa = mu_kappa + sigma_kappa * kappa_raw;             // bandit stickiness
}
  ...
  // Likelihood (reward weight anchored to 1.0)
  {
    ...
    for (s in 1:nSub) {
      for (rd in 1:nRd) {
        ...
        real a[nB];  // alpha
        real b[nB];  // beta
        int last_bandit;    // 0 = none, otherwise 1..nB (id of last chosen bandit)
        ...
        for (t in 1:nT) {
          if (choice[s,rd,t] != 0) {
            ...
            if (t >= phase2_start) {
              ...
              // add unscaled bandit stickiness: bonus if current bandit == last chosen bandit
              if (last_bandit == o1) logits[1] = logits[1] + kappa[s];
              if (last_bandit == o2) logits[2] = logits[2] + kappa[s];
              ...
              if (stim[s,rd,t] == 1)      a[bch] += 1;
              else if (stim[s,rd,t] == 0) b[bch] += 1;
              
              // update last chosen bandit for stickiness
              last_bandit = bch;
            }
            else {
              // base value signal (to be scaled by tau)
              V[1] = (Rmu[s,rd,t,o1] - 0.5);
              V[2] = (Rmu[s,rd,t,o2] - 0.5);
...
generated quantities {
  ...
  real gKappa = mu_kappa;  // mean bandit stickiness
