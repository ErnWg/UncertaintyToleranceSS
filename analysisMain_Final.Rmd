...
df.regression <- df.choice %>%
  ...
  dplyr::mutate(
    ...
    zSSS = scale(SSS),
    zBIS = scale(BIS),
    #dRmu = scale2_nocenter(dRmu),
    #dSsig = scale2_nocenter(dSsig),
    #dSmu = scale2_nocenter(dSmu),
    #dMotorSticky = scale2_nocenter(dMotorSticky)
  ) %>%
...
# -------- Then I want to plot the psychometric curves
################################
...
plot_model(
  tmp, 
  type = "pred",
  terms = c("dSsig[all]", "zSSS", "zBIS")
)

sjPlot::plot_model(tmp, type = "re")
...
fx_keep <- fx #if (any(fx$p_adj < alpha, na.rm = TRUE)) filter(fx, p_adj < alpha) else fx
...
ggplot(fx_plot, aes(x = beta, y = label, colour = sign)) +
  geom_errorbar(aes(xmin = lo, xmax = hi), orientation = "y", width = 0.15, linewidth = 1) +
  geom_point(size = 3) +
  geom_vline(xintercept = 0, linetype = "dashed") +
  scale_color_manual(values = cols, guide = "none") +
  labs(title = "Fixed effects (β)", x = "Coefficient (β)", y = NULL) +
  theme_classic(base_size = 12) +
  ggforce::facet_zoom(x = beta > 0.2 & beta < 3.1, horizontal = TRUE)
...
p_Rmu_SS <- ggplot(g, aes(x = x, y = predicted, color = group, fill = group, linetype = group)) +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high), alpha = 0.15, color = NA) +
  geom_line(linewidth = 1) +
  scale_color_manual(values = cols, guide = guide_legend(title = NULL)) +
  scale_fill_manual(values  = cols, guide = "none") +
  scale_linetype_manual(values = c("Low" = "dashed", "High" = "solid"), guide = "none") +
...
p_Ssig_SS <- ggplot(g, aes(x = x, y = predicted, color = group, fill = group, linetype = group)) +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high), alpha = 0.15, color = NA) +
  geom_line(linewidth = 1) +
  scale_color_manual(values = cols, guide = guide_legend(title = NULL)) +
  scale_fill_manual(values  = cols, guide = "none") +
  scale_linetype_manual(values = c("Low" = "dashed", "High" = "solid"), guide = "none") +
  labs(title = "ΔStim Uncertainty × SS interaction", x = "ΔStim Uncertainty", y = "P(Choice = Right)") +
  theme_classic(base_size = 12) + theme(legend.position = "top")
...
p_Ssig_BIS <- ggplot(g, aes(x = x, y = predicted, color = group, fill = group, linetype = group)) +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high), alpha = 0.15, color = NA) +
  geom_line(linewidth = 1) +
  scale_color_manual(values = cols, guide = guide_legend(title = NULL)) +
  scale_fill_manual(values  = cols, guide = "none") +
  scale_linetype_manual(values = c("Low" = "dashed", "High" = "solid"), guide = "none") +
  labs(title = "ΔStim Uncertainty × IP interaction", x = "ΔStim Uncertainty", y = "P(Choice = Right)") +
  theme_classic(base_size = 12) + theme(legend.position = "top")
...
p_Motor_SS <- ggplot(g, aes(x = x, y = predicted, color = group, fill = group, linetype = group)) +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high), alpha = 0.15, color = NA) +
  geom_line(linewidth = 1) +
  scale_color_manual(values = cols, guide = guide_legend(title = NULL)) +
  scale_fill_manual(values  = cols, guide = "none") +
  scale_linetype_manual(values = c("Low" = "dashed", "High" = "solid"), guide = "none") +
  labs(title = "Motor stickiness × SS interaction", x = "Motor stickiness", y = "P(Choice = Right)") +
  theme_classic(base_size = 12) + theme(legend.position = "top")
...
p_Motor_BIS <- ggplot(g, aes(x = x, y = predicted, color = group, fill = group, linetype = group)) +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high), alpha = 0.15, color = NA) +
  geom_line(linewidth = 1) +
  scale_color_manual(values = cols, guide = guide_legend(title = NULL)) +
  scale_fill_manual(values  = cols, guide = "none") +
  scale_linetype_manual(values = c("Low" = "dashed", "High" = "solid"), guide = "none") +
  labs(title = "Motor stickiness × BIS interaction", x = "Motor stickiness", y = "P(Choice = Right)") +
  theme_classic(base_size = 12) + theme(legend.position = "top")
...
fits <- list.files("results", pattern = ".stan.fit")
...
for (fit in 1:length(fits)){
  ...
  model <- readRDS(paste0("results/", fits[fit]))
...
models <- list(
  Reward    = readRDS("results/HBM_Reward.stan.fit.rds"),
  Theta   = readRDS("results/HBM_Theta.stan.fit.rds"),
  Omega   = readRDS("results/HBM_Omega.stan.fit.rds"),
  ThetaOmega  = readRDS("results/HBM_ThetaOmega.stan.fit.rds"),
  BanditSticky= readRDS("results/HBM_BanditSticky.stan.fit.rds"),
  MotorSticky= readRDS("results/HBM_MotorSticky.stan.fit.rds")
)
...
df.modelParams <- data.frame(
                ...
                mutate(log_tau = log(tau),
                       log_inv_tau = log(inv_tau),
                       e_omega = scale(omega*tau),
                       e_theta = scale(theta*tau),
                       #theta_omega = theta/omega,
                       #to_sum = abs(theta) + abs(omega),
                       #diff_theta_omega = theta - omega
                       ) #change to monotonic due to skeweddistribution
...
# --- Choose model/outcome (EDIT THESE TWO LINES ONLY) ------------------------
model       <- model_full_SSS       # or m3_BIS
outcome_var <- "s_SSS"      # or "s_BIS"

# --- Simple slope of ω across θ at τ = 0 -------------------------------------
tr <- emtrends(
  model,
  specs = ~ s_theta,   # θ is the moderator; condition on τ
  var   = "s_omega",
  at    = list(s_theta = g_theta),
  cov.reduce = mean                   # averages numeric covariates, incl. s_log_tau
)

tr0 <- summary(tr, infer = TRUE) |> as.data.frame()
trend_col <- names(tr0)[grepl("\\.trend$", names(tr0))]
...
# --- Prediction grid over ω × θ (hold covariates at typical values) ----------
nd <- expand_grid(s_omega = g_omega, s_theta = g_theta) |>
  mutate(
  ...
  left_join(tr_df, by = "s_theta")

nd$yhat <- predict(model, newdata = nd)

# --- Observed points near τ = 0 ----------------------------------------------
pts <- d |>
  transmute(s_omega, s_theta, y = .data[[outcome_var]])

# --- Fan plot (θ colour: blue -> red; solid lines where slope is significant) -
ggplot() +
  geom_point(data = pts, aes(s_omega, y, color = s_theta),
             alpha = 1, size = 1.6) +
  geom_line(data = nd, aes(s_omega, yhat, group = s_theta,
                           color = s_theta, linetype = sig_lab),
            linewidth = 0.5) +
  scale_color_gradient2(
    name = "θ (moderator)",
    low = "#005F73", mid = "grey90", high = "#AE2012",
    limits = range(d$s_theta, na.rm = TRUE),
    oob = scales::squish
  )+
  scale_linetype_manual(values = c("n.s." = "dotted", "p < .05" = "solid"),
                        name = "Simple slope of ω") +
  labs(
    x = "ω",
    y = paste("Predicted / observed SS"),
    title = paste0("SS ~ ω across θ")
  ) +
  theme_classic(base_size = 12)
...
# ---------------------- plot residual–residual (added-variable) ---------------
...
# --- Choose model/outcome (EDIT THESE TWO LINES ONLY) ------------------------
model       <- model_full_BIS       # or m3_BIS
outcome_var <- "s_BIS"      # or "s_BIS"
...
ggplot() +
  ...
  scale_linetype_manual(values = c("n.s." = "dashed", "p < .05" = "solid"),
                        name = "Simple slope of ω", guide="none") +
  labs(
    x = "ω",
    y = paste("Predicted / observed IP"),
    title = paste0("IP ~ ω across θ (moderator)")
  ) +
  theme_classic(base_size = 12)
